...
(function($) {
	...
	$.jCryption.getKeys = function(url, callback) {
		...
		base.getKeys = function() {
			$.getJSON(url,
					function(data) {
						keys = new base.jCryptionKeyPair(data.e, data.n,
								data.maxdigits);
	...
	$.jCryption.generatKeys = function(encryptionExponent, modulus, maxdigits) {
		setMaxDigits(parseInt(maxdigits, 10));
		this.e = biFromHex(encryptionExponent);
		this.m = biFromHex(modulus);
		this.chunkSize = 2 * biHighIndex(this.m);
	...
	$.jCryption.encrypt = function(string, keyPair, callback) {
		...
		function encryption(encryptObject) {
			...
			function encryptChar() {
				...
				var text = keyPair.radix == 16 ? biToHex(crypt) : biToString(
						crypt, keyPair.radix);
				encrypted += text + " ";
	...
	$.jCryption.defaultOptions = {
		submitElement : false,
		submitEvent : "click",
		getKeysURL : "EncryptionServlet?generateKeypair=true",
