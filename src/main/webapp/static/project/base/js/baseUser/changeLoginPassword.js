$(function() {
	 ...
	 * 显示密码修改规则
	 */
	$(".updatePassword-area .pwd-limit").text(baseUtil.getPasswordLimit());

	/**
	 * 基本资料
	 */
	base.editWin.editWinInput(baseUtil.val("N.originPassword"),
			"originPassword", "originPassword",
			$("#section-floor-1 .panel-body"), baseUtil.validateAttr("true",
					null, null, baseUtil.val("E.required")), "260px", "", true);
	...
	$("#changePassword").click(
			function() {
				if (changePasswordValidate.validate()) {
					baseUtil.saveAJ(ctx + "/baseUserWS/changePassword", {
						"originPassword" : baseUtil.encryptPasswordByPPK($
								.trim($("#originPassword").val()), keyPair),
